...
class Genotype:
    ...
    def random_genotype(self, low_lim, up_lim, dimension):
        return cp.random.randint(low=low_lim, high=up_lim, size=dimension)
    ...
    def Mutation(self, population):
        ...
        for i in range(len(population)):
            indexes = cp.arange(0, len(population))
            indexes = indexes[indexes != i]
            new = cp.random.choice(indexes, 3, replace=False)
